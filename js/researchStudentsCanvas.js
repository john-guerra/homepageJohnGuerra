...
function doNetwork(data) {
  ...
  var simulation = d3
    ...
    .force("x", d3.forceX(width / 2).strength((width / height) * 0.1))
    .force("y", d3.forceY(height / 2).strength((height / width) * 0.1))
...
function doProjectList(data) {
  ...
  function createStudent(sel) {
    ...
    var nameAndPhoto = row.append("div").attr("class", "col-xs-4");
    ...
    var body = row.append("div").attr("class", "col-xs-8");
    body
      .append("h3")
...
function movePhoto(nick) {
  ...
  d3.select(".node#" + nick)
    ...
    .tween("style.top", function() {
      var target = boundingRect.top - 50 + window.pageYOffset,
        // nSel = this,
