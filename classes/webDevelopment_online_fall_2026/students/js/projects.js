...
/**
 * Restrict hrefs to safe schemes. The project data is student-submitted via a
 * Google Form, so a malicious value like `javascript:...` must never end up in
 * an href. Anything that isn't http/https/mailto is neutralized to "#".
 */
...
/**
 * Validate a thumbnail URL before interpolating it into a CSS url() value.
 * Rejects anything that isn't a plain http(s) URL (no quotes, parens, or
 * whitespace) to prevent CSS-injection breakout from the spreadsheet data.
 */
function safeThumbUrl(url) {
  return /^https?:\/\/[^\s'"()<>]+$/.test(url || "") ? url : "";
}

// Published CSV of the Fall 2026 project submissions form responses.
let urlCSV =
  "https://docs.google.com/spreadsheets/d/e/2PACX-1vSa3f4Z4Lh1crPY11kvSTBImO8skUB3tfTNgUU5-wXuXieX2G3RYd988PcSzZWijm_azGFNNSbOgAUr/pub?gid=1483540304&single=true&output=csv";
...
/**
 * Build a DOM-id-safe slug for a section key (used for aria-controls).
 */
function sectionId(key) {
  return "section-" + String(key).toLowerCase().replace(/[^a-z0-9]+/g, "-");
}

/**
 * Sync the accessibility state of a topic node to its collapsed state:
 * - aria-expanded on the toggle button
 * - `inert` on the content wrapper so collapsed cards leave the tab order
 *   AND the screen-reader tree (opacity/height:0 alone do not).
 */
function updateSectionA11y(topicNode) {
  const topic = d3.select(topicNode);
  const collapsed = topic.classed("collapsed");
  topic.select(".topic-toggle").attr("aria-expanded", collapsed ? "false" : "true");
  const wrapper = topic.select(".projects-grid-wrapper");
  wrapper.attr("inert", collapsed ? "" : null);
}

/**
 * Announce the current result count to screen readers via the live region.
 */
function announceResults() {
  const status = document.getElementById("results-status");
  if (!status) return;
  const n = State.filteredData.length;
  status.textContent = n === 0 ? "No projects found." : `${n} projects shown.`;
}
...
function expandAll() {
  ...
  container.selectAll(".topic").each(function () {
    updateSectionA11y(this);
  });
  updateURL();
}
...
function updateScrollParallax() {
  ...
  // Respect reduced-motion preference: skip the scroll-driven parallax
 ...
 */
/**
 * Copy shown when there are no project cards to display.
 *
 * TODO (John): decide what an empty Fall 2026 roster should say before the
 * first submissions land. State.allData.length === 0 means "semester hasn't
 * produced submissions yet"; a non-zero allData with zero filteredData means
 * the user's search matched nothing. Returns an HTML string.
 */
function emptyStateHtml() {
  if (State.allData.length === 0) {
    // TODO: message for "no submissions yet this semester"
  }
  return `<p>No projects found matching your search.</p><p class="text-muted">Try a different search term.</p>`;
}
...
function render() {
  ...
  // Two different reasons the list can be empty, and they need different copy:
  //   1. State.allData is empty      -> no submissions exist yet this semester
  //   2. State.allData has rows but  -> the search/filter excluded everything
  //      State.filteredData is empty
  if (State.filteredData.length === 0) {
    container.append("div").attr("class", "no-results").html(emptyStateHtml());
    return;
  }
  ...
  // Section header: an <h2> (document outline) wrapping a real <button> so it
  // is keyboard-operable and exposes aria-expanded / aria-controls. A native
  // button gives Enter/Space activation for free; the click bubbles to the h2.
  const headers = topics.append("h2").attr("class", "topic-header");

  headers.html(
    (d) =>
      `<button type="button" class="topic-toggle" aria-expanded="${
        State.collapsedSections.has(d.key) ? "false" : "true"
      }" aria-controls="${sectionId(d.key)}">${Icons.chevron}<span class="topic-title">${
        d.key
      }</span><span class="project-count">${d.values.length} projects</span></button>`
  );
  ...
  // The wrapper is what animates open/closed (grid-template-rows 1fr <-> 0fr),
  // so the inner grid is never height-capped and cards are never clipped.
  // `inert` when collapsed keeps hidden cards out of the tab order / a11y tree.
  const gridWrappers = topics
    .append("div")
    .attr("class", "projects-grid-wrapper")
    .attr("id", (d) => sectionId(d.key))
    .attr("inert", (d) => (State.collapsedSections.has(d.key) ? "" : null));
  const grids = gridWrappers.append("div").attr("class", "projects-grid");
  ...
  const cards = grids
    ...
    .style("--thumbnail-url", (d) => {
      const t = safeThumbUrl(d[Project.THUMBNAIL]);
      return t ? `url("${t}")` : "none";
    });
  ...
  thumb
    .append("a")
    .attr("href", (d) => safeHref(d["Project URL"] || d["Github repo URL"]))
    .attr("target", "_blank")
  ...
  studentName
    .append("a")
    .attr("href", (d) => safeHref(d["Personal homepage URL 1"]))
    .attr("target", "_blank")
    .attr("rel", "noopener noreferrer")
  ...
  info
    ...
    .append("a")
    .attr("href", (d) => safeHref(d["Personal homepage URL 2"]))
    .attr("target", "_blank")
    .attr("rel", "noopener noreferrer")
  ...
  actions
    ...
    .attr("href", (d) => safeHref(d["Github repo URL"]))
    .attr("target", "_blank")
    .attr("rel", "noopener noreferrer")
  ...
  actions
    ...
    .attr("href", (d) => safeHref(d["Project URL"]))
    .attr("target", "_blank")
    .attr("rel", "noopener noreferrer")
  ...
  actions
    ...
    .attr("href", (d) => safeHref(d["Public video URL"]))
    .attr("target", "_blank")
    .attr("rel", "noopener noreferrer")
  ...
  actions
    ...
    .attr("href", (d) => safeHref(d["Google Slides URL"]))
    .attr("target", "_blank")
    .attr("rel", "noopener noreferrer")
