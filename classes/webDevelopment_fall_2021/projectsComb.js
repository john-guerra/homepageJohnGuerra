...
function ProjectsComb() {
  ...
  var projects = svg.append("g").attr("class", "projects");
  var mesh = svg.append("path").attr("class", "mesh");

  //Function to call when you mouseover a node
  function mover(d) {
    if (!d.important) return;
    d3.select(this).transition().duration(10).style("opacity", 0.3);
    console.log(d);
  }
  ...
  function updateComb(data) {
    ...
    var centers = hexbin.radius(hexRadius).size([width, height]).centers();

    d3.select(containerID + " > svg #hexClip > path")
      .attr(
        "transform",
        "translate(" + Math.sin(Math.PI / 3) * hexRadius + "," + hexRadius + ")"
      )
      .attr("d", function () {
    ...
    var fillingI = 0,
      importantI = data.length - 1; // Two indexes to fill the comb
    ...
    centers.forEach(function (center, i) {
      ...
      center.i = Math.round(
        (center[0] - (center.j & 1) * hexRadius * Math.sin(Math.PI / 3)) /
          (hexRadius * 2 * Math.sin(Math.PI / 3))
      );
      ...
      if (proj) proj.index = i;
      center.proj = proj;
    });

    var projectsSel = projects.selectAll(".projectHex").data(centers);

    var projectsSelEnter = projectsSel
      .enter()
      .append("g")
      .attr("class", "projectHex");

    projectsSelEnter.merge(projectsSel).classed("important", function (d) {
      return d.important;
    });
    ...
    projectsSelEnter
      ...
      .attr("transform", function (d) {
        return (
          "translate(" +
          (d.x + Math.sin(Math.PI / 3) * hexRadius) +
          "," +
          (d.y + hexRadius) +
          ")"
        );
